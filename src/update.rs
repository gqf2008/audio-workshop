        ...
        ureq::Error::Status(code, resp) => {
            ...
            let hint = match code {
                404 => {
                    ...
                    // 把"仓库不可匿名读"误导成"你填错了"。这是通用文案：任何用户都可能把清单
                    // 地址指向私有仓库。（本项目的仓库 2026-09-18 起已转 public，别再写"本项目
                    // 就是私有的"——那句已经过期了。）
...
mod tests {
    ...
    fn check_surfaces_http_status_and_body() {
        ...
        // 仓库私有时 GitHub 匿名 API 回的就是 404（不是 403），这正是本项目默认地址的处境：
        // 只说"地址不对或没有 Release"会把用户引到"我填错了"的错误结论上。
        assert!(
            e.contains("私有仓库也回 404"),
            "404 要说清私有仓库也回 404：{e}"
        );
        // 拼接长文案时最容易留下"空白断层"（曾出现连续 21 个空格）。这条是本项目默认地址下
        // **必然**上屏的文案，所以在这里钉死：用户可见的错误串里不许有连续空格。
