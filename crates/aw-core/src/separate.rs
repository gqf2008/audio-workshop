...
///
/// 为什么需要它（2026-09-17 实测）：上游把输入样本**原样**输出，但产物 WAV **一律声明 44.1kHz**。
/// 48kHz 输入因此得到一个"同帧数、44.1kHz"的产物——**时长 +8.84%、播放被拉慢 1.0884 倍**。
...
///
/// 上游只错了标签：它喂给模型的是"把输入样本当 44.1kHz 播"的慢放版本，产物样本与输入样本
/// 一一对应，所以改回输入的标签是无损往返（比真做一次重采样更保真——不引入抗混叠滤波损失）。
/// 标签已经一样、或不是 16-bit PCM 时原样返回。
