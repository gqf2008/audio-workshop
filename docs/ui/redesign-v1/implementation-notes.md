# 音频作坊 · UI Redesign v1 实现说明

> 本文只给出从设计到 Rust / Slint 的映射、callback/属性增删建议、迁移步骤与验收标准，不修改实现、不提交代码。  
> 实施基线：当前 `feat/ui-simplify` 实际是“配音 + BGM”两 Tab；`feat/m4-song` 另有 `SongWorkbench`，但主界面模型与命名不符合本轮设计。实施时应先把两条线对齐，再做壳层重构。

## 1. 现状事实

| 项目 | 当前事实 | 对本设计的影响 |
|---|---|---|
| 场景导航 | `ui/app.slint` 当前 `scenes = ["配音", "BGM"]`；顶部有 segmented tab | 保留顶部导航，扩展为四个场景 |
...
| 组件 | 位置建议 | 职责 |
|---|---|---|
| `TopTabBar` | `ui/shell.slint` | 四个一级 Tab；选中态、运行/失败 badge、键盘切换 |
...
| `PixelSegmentedControl` | 顶部四 Tab 可用；若选中态在窄屏不稳定，另做 `TopTabBar` 包装 |

`build.rs` 当前只监听 `ui/app.slint`、`ui/dub_workbench.slint`、`ui/model.slint`；新增 `shell.slint`、`task_center.slint`、`export_drawer.slint`、`separation_workbench.slint`、`music_workbench.slint` 后必须同步增加 rerun 监听。此项属于实施步骤，不在本轮设计改动中执行。
...
```text
scenes: ["配音", "BGM"]                    → ["配音", "BGM", "人声分离", "音乐制作"]
scene 索引                                  → 0..3，并在 UI state 持久化
...
| `drawer-scope` | string | `dub` / `bgm` / `separation` / `music` / `export` |
| `capability-separation` | string/bool | 分离后端未就绪时做能力门控 |
| `capability-music` | string/bool | 音乐制作实验能力门控 |
| `music-result`、`separation-result` | struct/compatible model | 结果条和导出读取 |
...
```text
id
scene             配音 / bgm / separation / music
kind              synthesize / bgm_generate / mix / separate / music_generate / export
...
3. 配音高级、BGM 高级、分离高级、音乐高级各自只渲染自己的组。
...
- [ ] 960×640 和 1200×880 下，顶部四个 Tab 都可见且不换行。
- [ ] 四个 Tab 均可在一次点击内到达；任何 Tab 都不藏在抽屉或二级页。
- [ ] 切 Tab 不取消正在运行的合成、BGM、分离或音乐任务。
...
- [ ] 每个稳定状态只有 1 个主操作。
- [ ] 首屏可见参数输入数量：配音 0、BGM 1、分离 1、音乐 2。
...
- [ ] 任务中心能看到四个来源的 queued / running / failed / done 状态。
- [ ] 失败任务可重试并回到来源 Tab。
...
另外增加 UI 状态截图基线和四个 Tab 的尺寸快照；每个状态至少覆盖 960×640 窄窗。若新增 capability gate 或任务恢复路径，按仓库规则补阳性/阴性测试，确保未就绪态和失败态真实可达。
