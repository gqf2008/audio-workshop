...
### Step 5：迁移音乐制作 —— ✅ 已完成（2026-09-16，提交见 feat/music-tab）

1. ✅ `SongWorkbench` 标题改「音乐制作」；「彩蛋」改为副标题里的一句质量口径 + 状态行免责提示。
2. ✅ 引擎（yue2 / ACE-Step）移入本页「高级」（页内展开盒子，与配音页同款）。
3. ✅ 首屏保留歌词、风格、`生成歌曲`、`高级`；`试听` / `导出 WAV` 合并成结果条（有成品才出现）。
   计数：默认态 4 + 外壳 3 = 7；结果就绪 5 + 3 = 8，达标（原为 7 + 3 = 10）。
4. ⏳ 统一任务队列仍未接：歌曲任务目前仍是独立的 `Cmd::RunSong` 通道，
   生成中只锁定本页控件，没进跨 Tab 的任务中心。
