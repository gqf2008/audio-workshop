...
| 配音合成（单篇） | ❌ | 会改写 worker 持有的 `current` 工程；两个 Run 同时飞会互相覆盖 |
| 配音合成（批量，见 `docs/batch-queue.md`） | ✅ | 每篇各落自己的工程目录、按顺序跑，不共享 `current`；提交时按条登记成 N 条配音任务 |
...
因此截图/演示态（`AW_UI_STATE=tasks`）里**不会**出现停止按钮：演示任务没有占用真实槽位。

演示数据仍会进台账（那是任务中心的展示来源），但 `tasks_in_flight` 在演示态直接返回
false——**只清槽位不够**：台账本身也是提交守卫的判据，否则假的 Running/Pending 条目会把
真实的"开始配音 / 生成 BGM"挡下来（复核抓到过这一条）。要看到停止按钮就跑一条真任务。
