...
| 配音合成 / 重录 / 拼装 | ❌ | 会改写 worker 持有的 `current` 工程；两个 Run 同时飞会互相覆盖 |
...
- 取消是**协作式**的：正在跑的任务只能在各自的检查点响应（人声分离按分块、BGM 按分段、
  配音按句）。排队中的任务则是硬取消——`take` 到取消标记就不执行。
- **歌曲没有运行中停止**：`generate_song` 是一次阻塞调用，服务端没有取消接口，
  请求发出去就中断不了。所以歌曲 Tab 的主按钮只在**排队中**显示"取消排队"，
  一旦开始跑就显示"生成中…"（不给假停止）。人声分离有分块检查点，运行中仍可停。
