...
class DeclaredWeightsComposeTests(unittest.TestCase):
    ...
    `gen.APP_SUPPORTS_MULTI_FILE_ENTRY` 后面：app 侧支持多文件入口后已置 True（默认组条目），
    关掉时（历史/回退路径）必须如实 no-source + 写明阻塞点。两侧都要有用例，免得开关悄悄失效。
    """
    ...
    def test_declared_weights_are_composed_across_packages(self):
        """yue2 形态：主权重与 vae 分属两个包 → 跨包组出**一条多文件**条目。

        两侧都钉：默认（app 已支持多文件入口）组条目；开关关掉时如实 no-source 并点名阻塞点。
        """
        spec = _dir_spec(
            "yue2",
            [
        ...
        # 默认（app 侧已支持多文件入口）：主包取最早声明键命中的那个，vae 跨包补齐
        self.assertTrue(gen.APP_SUPPORTS_MULTI_FILE_ENTRY, "app 侧支持多文件入口后这个开关应当已打开")
        ...
        self.assertEqual(
            names[:2], ["yue2-3b-q4_0.gguf", "yue2-vae-f16.gguf"], "声明的权重必须排在前面（主文件=权重）"
        )
        self.assertIn("a.json", names, "配套 sidecar 也要一起下（缺了同样跑不起来）")
        urls = [f["url"] for f in m["entry"]["files"]]
        self.assertEqual(len(urls), len(set(urls)), "跨包补齐必须按 URL 去重")
        self.assertIn("跨包补齐", m["note"])

        # 开关关掉（历史/回退路径）：不许给出"清单说能下、界面点不动"的入口
        gen.APP_SUPPORTS_MULTI_FILE_ENTRY = False
        m = self._model(schema_model, {"yue2": spec})
